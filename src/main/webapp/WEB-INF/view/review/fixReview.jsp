...
function init() {
	...
	$('#registerBtn').click(() => {
		if($('#title').val()) {
			$.ajax({
				url: '${pageContext.request.contextPath}/review/fixReview/' + $('#reviewNum:checked').val(),
				type: 'put',
				data: {
					title: $('#title').val(),
					content: $('#content').val()
				...
				$('#modalTitle').text('후기 수정');
				$('#modalMsg').text('글이 수정되었습니다.');
				$('#cofirmModal').modal();
				$('#noBtn').hide();
				$('#okBtn').hide();
			})
           ...
           <f:form modelAttribute='review' id='loginForm' method='post'>
			...
			<div style='float:right; margin-top:10px;'>
				...
				<button class='btn btn-outline-secondary btn-sm' id='registerBtn' 
					data-toggle='modal' data-target='#registerModal'>수정</button>
			</div>
		</div>
	</div>
</div>
